...
this.options.columnClass);1<a?this.calculateShortest():this.minColumn=this.$columns[0];return this},calculateShortest:function(a,b){b&&b.height(a+b.height());var e=this.$columns,d=[],d=e.map(function(a,b){return jQuery(b).height()});this.minColumn=e.map(function(a,b){if(jQuery(b).height()==Math.min.apply(null,d))return b})[0];this.maxColumn=e.map(function(a,b){if(jQuery(b).height()==Math.max.apply(null,d))return b})[0];this.$grid.height(jQuery(this.maxColumn).height());console.log(d)},calculateGrid:function(a){this.windowWidth=
jQuery(i).width();this.gridWidth=this.$grid.innerWidth();console.log(this);this.gutter=this.analyzeUnits(this.options.layout.gutter,this.gridWidth);360>=this.windowWidth?this.layout="mobile-vertical":480>=this.windowWidth&&360<this.windowWidth?this.layout="mobile-horizontal":768>=this.windowWidth&&480<this.windowWidth?this.layout="ereader":980>=this.windowWidth&&768<this.windowWidth?this.layout="tablet-vertical":1024>=this.windowWidth&&980<this.windowWidth?this.layout="tablet-horizontal":1200>=this.windowWidth&&
