...
this.blockNumber){var e=this,d=0;a>=this.columnNumber&&(d=this.gutter);var c=jQuery(this.$blocks[a]).attr("id",this._defaults.blockClass+"-"+(a+1)).addClass(this._defaults.blockClass).css({margin:0,padding:0,marginTop:d}),g=c.find("img"),f=g.length,h=this.minColumn,i=jQuery(h),j=0,k=this.options.duration,l=function(){j=c.height()+d;b>1&&e.calculateShortest(j,i);e.options.onOneLoad();e.renderBlock(a+1,b)};0===a&&this.$grid.fadeIn();c.hide().appendTo(h);0<f?c.fadeIn(k,function(){g.one("load",l).each(function(){this.complete&&
l()})}):c.fadeIn(k,function(){l()});m++}else if(a==this.blockNumber)this.options.onAllLoad()},analyzeUnits:function(a,b){var e=a.toString();return-1<e.search("px")||-1<e.search("em")?a:parseFloat(a)/100*b},createColumns:function(a){var b,e=this.columnWidth,d=this.gutter,c=this.options.columnClass;for(b=0;b<a;b++){var g=jQuery("<div></div>").attr("id",c+"-"+(b+1)).addClass(c);1<a&&g.css({position:"absolute",width:e,left:e*b+d*b,height:0});this.$grid.append(g)}this.$columns=jQuery("."+this.options.columnClass);
1<a?this.calculateShortest():this.minColumn=this.$columns[0];return this},calculateShortest:function(a,b){b&&b.height(a+b.height());var e=this.$columns,d=[],d=e.map(function(a,b){return jQuery(b).height()});this.minColumn=e.map(function(a,b){if(jQuery(b).height()==Math.min.apply(null,d))return b})[0];this.maxColumn=e.map(function(a,b){if(jQuery(b).height()==Math.max.apply(null,d))return b})[0];this.$grid.height(jQuery(this.maxColumn).height())},calculateGrid:function(a){this.windowWidth=jQuery(i).width();
this.gridWidth=this.$grid.parent().width();this.gutter=this.analyzeUnits(this.options.layout.gutter,this.gridWidth);360>=this.windowWidth?this.layout="mobile-vertical":480>=this.windowWidth&&360<this.windowWidth?this.layout="mobile-horizontal":768>this.windowWidth&&480<this.windowWidth?this.layout="ereader":980>this.windowWidth&&768<=this.windowWidth?this.layout="tablet-vertical":1024>=this.windowWidth&&980<this.windowWidth?this.layout="tablet-horizontal":1200>=this.windowWidth&&1024<this.windowWidth?
