...
jQuery("<div/>").addClass(this.options.gridClass).css({position:"relative"});jQuery(this.element).height(0);this.$grid=jQuery(this.element).wrap(b);this.$blocks=[];null!==this.options.blocks?(this.$blocks=this.options.blocks,this.type="dynamic"):(jQuery(this.element).hide(),this.$blocks=jQuery(this.element).children(),this.type="static");this.blockNumber=this.$blocks.length;this.calculateGrid("initial").createColumns(this.columnNumber).renderBlock(0,this.columnNumber);jQuery(i).resize(function(){var b=
a.columnNumber,d=a.calculateGrid(),c=d.columnNumber;if(b!=c)i.location.reload();else{b=d.$columns;b.each(function(){jQuery(this).children().each(function(a){a>=c&&jQuery(this).css({marginTop:d.gutter})})});if(c>1){var g=[];b.each(function(a){var b=jQuery(this).children().eq(0),c=jQuery(this).children().last(),c=c.position().top+c.outerHeight(true);g.push(c);jQuery(this).css({height:c,width:d.columnWidth,left:d.columnWidth*a+d.gutter*a});b.css({marginTop:0})});d.$grid.height(Math.max.apply(null,g))}}})},
renderBlock:function(a,b){if(a<this.blockNumber){var e=this,d=0;a>=this.columnNumber&&(d=this.gutter);var c=jQuery(this.$blocks[a]).attr("id",this._defaults.blockClass+"-"+(a+1)).addClass(this._defaults.blockClass).css({margin:0,padding:0,width:"100%",marginTop:d}),g=c.find("img"),f=g.length,h=this.minColumn,i=jQuery(h),j=0,k=this.options.duration,l=function(){j=c.outerHeight()+d;b>1&&e.calculateShortest(j,i);e.options.onOneLoad();e.renderBlock(a+1,b)};0===a&&e.$grid.fadeIn();c.appendTo(h).hide();
0<f?(c.delay(k*a).fadeIn(k),g.one("load",l).each(function(){this.complete&&l()})):(c.delay(k*a).fadeIn(k),l());n++}else if(a==this.blockNumber)this.options.onAllLoad()},analyzeUnits:function(a,b){var e=a.toString();return-1<e.search("px")||-1<e.search("em")?a:parseFloat(a)/100*b},createColumns:function(a){var b,e=this.columnWidth,d=this.gutter,c=this.options.columnClass;for(b=0;b<a;b++){var g=jQuery("<div></div>").attr("id",c+"-"+(b+1)).addClass(c);1<a&&g.css({position:"absolute",width:e,left:e*b+
d*b});this.$grid.append(g)}this.$columns=jQuery("."+this.options.columnClass);1<a?this.calculateShortest():this.minColumn=this.$columns[0];return this},calculateShortest:function(a,b){b&&b.height(a+b.height());var e=this.$columns,d=[],d=e.map(function(a,b){return jQuery(b).height()});this.minColumn=e.map(function(a,b){if(jQuery(b).height()==Math.min.apply(null,d))return b})[0];this.maxColumn=e.map(function(a,b){if(jQuery(b).height()==Math.max.apply(null,d))return b})[0];this.$grid.height(jQuery(this.maxColumn).height())},
